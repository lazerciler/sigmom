...
// Locale-aware, backend-derived START_CAPITAL, no quote inference.
(() => {
    // Değişiklik izleme
    ...
    function groupDailyPnL(rows) {
        ...
        return {
            days,
            dayPnls: days.map((k) => map.get(k) || 0)
        };
    }
    ...
    document.addEventListener("DOMContentLoaded", async () => {
        ...
        try {
            document.getElementById('equity-kpi')?.classList.remove('hidden');
        } catch {}

        // Başlangıç sermayesini (bakiye) backend'den oku, sonra çiz
        await initStartCapital();
        try {
            window.START_CAPITAL = START_CAPITAL;
        } catch {}
