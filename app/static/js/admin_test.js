...
async function refreshStatus() {
    try {
        ...
        // Exchange bilgisini otomatik olarak set et
        if (j.exchange) {
            el('exchangeDisplay').textContent = j.exchange;
            onExchangeAutoUpdate(j.exchange);   // ← payload/preview senkronizasyonu
...
/** Exchange otomatik değişince payload/preview’i senkronize et */
function onExchangeAutoUpdate(newEx) {
    const hid = el('exchange');
    if (!hid) return;
    const prev = hid.value;
    if (prev === newEx) return;   // değişmemiş
    hid.value = newEx;
    // Raw JSON edit açıksa, içindeki "exchange" alanını da güncelle
    if (el('rawMode')?.checked) {
        try {
            ...
            // parse hatası varsa dokunma, sadece preview’i yenile
        }
    }
    refreshPreview();
    flash(el('exchangeDisplay'));
}
...
function fmtIsoLocal(d) {
    ...
    const abs = Math.abs(off),
        oh = pad(Math.floor(abs / 60)),
        om = pad(abs % 60);
    return `${y}-${m}-${day}T${hh}:${mm}:${ss}${sign}${oh}:${om}`;
}

function nowIsoLocal() {
    return fmtIsoLocal(new Date());
}
...
function nowIsoByMode() {
    return getTzMode() === 'utc' ? nowIsoUtc() : nowIsoLocal();
}
...
async function sendSignal() {
    ...
    try {
        ...
        el('respOut').textContent = JSON.stringify({
            error: String(e)
        }, null, 2);
        setRespBadge(false);
        setStatus('idle');
...
el('btnSend').addEventListener('click', (e) => {
    e.preventDefault();
    if (!el('symbol').value.trim()) {
        alert('Symbol gerekli');
        return;
    }
    if (!el('position_size').value.trim()) {
        alert('Position size gerekli');
        return;
    }
    if (!el('fund_manager_id').value.trim()) {
        alert('Fund manager id gerekli');
        return;
    }
    sendSignal();
});
