...
(() => {
  ...
  function setEntryLine(side, price){
    ...
    // eskisini kaldır
    try { entryLines[s] && candle.removePriceLine(entryLines[s]); } catch {}
    entryLines[s] = null;
    // yeni çiz
  ...
  function updateEntryLines(map){ // { long?: number, short?: number }
    ...
    else { try { entryLines.long && candle.removePriceLine(entryLines.long); } catch {} entryLines.long = null; }

    // Short
    if (drawShort != null) setEntryLine('short', drawShort);
    else { try { entryLines.short && candle.removePriceLine(entryLines.short); } catch {} entryLines.short = null; }
  }
  function clearEntryLines(){
    try { entryLines.long  && candle.removePriceLine(entryLines.long); } catch {}
    try { entryLines.short && candle.removePriceLine(entryLines.short); } catch {}
    entryLines = { long: null, short: null };
  }
  // geriye dönük: tek çizgi temizleme çağrıları boşa düşmesin
  function clearEntryLine(){ clearEntryLines(); }
...
function ensureMaChart() {
  ...
  if (existing) { maChart = existing; return maChart; }
  // 2) Boyutu kilitle
  ...
  maChart = new Chart(el.getContext('2d'), {
    ...
    options: {
      ...
      scales:{ r:{
        ...
        grid:{ color: dark?'rgba(148,163,184,.18)':'rgba(100,116,139,.22)' },
        angleLines:{ color: dark?'rgba(148,163,184,.22)':'rgba(100,116,139,.22)' },
        pointLabels:{
...
function updateMaConfluence(series){
  const ch = ensureMaChart(); if(!ch) return;
  if(!Array.isArray(series) || series.length < 2){
    ch.data.datasets[0].data = [0,0,0]; ch.update('none'); return;
  }
  // Kapanmış son bar
...
const onTheme = (e) => {
  ...
  try { applyChartTheme(dark); } catch {}
  try { window.EQUITY_REDRAW?.(); } catch {}
  try { applyMaChartTheme(dark); } catch {}
};
...
// snap & nudge helpers
function snapToBar(sec, tfSec) { return Math.floor(sec / tfSec) * tfSec; }
  ...
  function groupSameBar(baseList) {
    ...
    for (const m of baseList) {
      ...
      if (!map.has(key)) map.set(key, { ...m, text: baseText, __count: 0 });
      map.get(key).__count++;
    }
...
// === EOF Markers ===
  ...
  // ==== market verisi ====
async function loadKlines({ symbol = getCurrentSymbol(), tf = klTf, limit = klLimit } = {}) {
    try {
      ...
      if (series.length) {
        ...
        if (document.getElementById('maConfluence') && window.Chart) {
          try { updateMaConfluence(series); } catch {}
        }
    ...
    const markers = rows
      ...
      .map(m => {
        ...
        if (kind === 'open') {
          ...
          if (side === 'long') {
            // Long
            shape = 'circle'; color = '#047857'; pos = 'aboveBar'; size = 1;
          } else if (side === 'short') {
            // Short
            shape = 'circle';   color = '#b91c1c'; pos = 'belowBar'; size = 1;
          } else {
            shape = 'circle';    color = '#f59e0b'; size = 1; // bilinmiyorsa turuncu daire
          }
        }
...
async function loadOpenTrades() {
  try {
    ...
    if (!Array.isArray(items) || items.length === 0) {
      ...
      clearEntryLines();           // ⟵ boş liste: entry çizgilerini temizle
      return [];
    }
    if (!el) return items;
    ...
    if (GUEST) { clearEntryLines(); return items; }
    try {
      ...
      const perSide = { long: null, short: null };
      if (rows.length) {
        ...
        if (longRows.length) {
          ...
          if (!Number.isFinite(v)) for (const r of [...longRows].reverse()) { v = pickFrom(r); if (Number.isFinite(v)) break; }
          if (Number.isFinite(v)) perSide.long = v;
        }
        if (shortRows.length) {
          let v = pickFrom(shortRows[shortRows.length - 1]);
          if (!Number.isFinite(v)) for (const r of [...shortRows].reverse()) { v = pickFrom(r); if (Number.isFinite(v)) break; }
...
async function loadRecentTrades() {
  try {
    ...
    // Equity modülüne haber ver
    try {
      document.dispatchEvent(new CustomEvent('sig:recent-trades', { detail: { items } }));
    } catch (e) {}
  ...
  async function loadSymbols() {
    ...
    try { const r = await fetch('/api/me/symbols', { credentials: 'include' });
  ...
  async function setTimeframe(tf) {
    ...
    await loadMarkers({ symbol: activeSymbol, tf: klTf });
  }
  ...
  (function wireQuickBalance(){
    ...
    function extractUpnlBySide(payload, symbol, openRowsForActive = []) {
      const SYM = String(symbol||'').toUpperCase();
      const out = { long: 0, short: 0, hasLong: false, hasShort: false };
      ...
      if (Array.isArray(arr)) {
        for (const p of arr) {
          ...
          if (side === 'short') { out.short += +u; out.hasShort = true; }
          else { out.long += +u; out.hasLong = true; }
        }
        return out;
      }
      ...
      if (side === 'short') { out.short = agg; out.hasShort = true; }
      else { out.long = agg; out.hasLong = true; }
      return out;
    }
    ...
    try {
      ...
      const qsSum = new URLSearchParams({ limit: '1000' });
      ...
      if (rSum.ok) {
        ...
        if (openCnt === 0) {
          try {
            ...
            if (r2.ok) {
              ...
              if (Array.isArray(arr2)) { openRowsAll = arr2; openCnt = arr2.length; }
            }
          } catch {}
        }
        ...
        if (openCnt > 0) {
            ...
            let ju = null;
            const qs0 = new URLSearchParams({ symbol: SYM });
            if (ex) qs0.set('exchange', ex);
            let rU = await fetch('/api/me/unrealized?' + qs0.toString(), { credentials:'include', headers:{'Accept':'application/json'} });
            if (rU.ok) {
              ju = await rU.json();
            ...
            if (!bd.hasLong && !bd.hasShort) {
              const qs2 = new URLSearchParams({ symbol: SYM, all: '1' });
              if (ex) qs2.set('exchange', ex);
              rU = await fetch('/api/me/unrealized?' + qs2.toString(), { credentials:'include', headers:{'Accept':'application/json'} });
              if (rU.ok) {
                ju = await rU.json();
                bd = extractUpnlBySide(ju, SYM, openRowsForActive);
                lastPayload = ju;
              ...
              // fallback: aggregate
              if (!bd.hasLong && !bd.hasShort) {
                const qsA = new URLSearchParams({ symbol: SYM });
              ...
              if (!bd.hasLong && !bd.hasShort && openRowsForActive.length) {
                ...
                if (sideFromOpen === 'short') { bd.short = total; bd.hasShort = true; }
                else { bd.long = total; bd.hasLong = true; }
              }
            ...
            if (rowUpL && elUpL) {
              if (bd.hasLong) {
                const v = Number(bd.long || 0);
                elUpL.classList.remove('text-emerald-500','text-rose-400');
                if (v > 0) elUpL.classList.add('text-emerald-500');
                else if (v < 0) elUpL.classList.add('text-rose-400');
                const sign = v > 0 ? '+' : (v < 0 ? '−' : '');
                elUpL.textContent = sign + nf2QB.format(Math.abs(v));
                rowUpL.classList.remove('hidden');
              } else {
                // LONG bacağı yoksa satırı gizle
                rowUpL.classList.add('hidden');
              }
            }
            if (rowUpS && elUpS) {
              if (bd.hasShort) {
                const v = Number(bd.short || 0);
                elUpS.classList.remove('text-emerald-500','text-rose-400');
                if (v > 0) elUpS.classList.add('text-emerald-500');
                else if (v < 0) elUpS.classList.add('text-rose-400');
                const sign = v > 0 ? '+' : (v < 0 ? '−' : '');
                elUpS.textContent = sign + nf2QB.format(Math.abs(v));
                rowUpS.classList.remove('hidden');
              } else {
                // SHORT bacağı yoksa satırı gizle
