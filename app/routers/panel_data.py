...
async def markers_public(
    symbols: Optional[str] = Query(None, description="Virgülle ayrılmış semboller (örn: BTCUSDT,ETHUSDT)"),
    ...
    q_open = select(StrategyOpenTrade).where(func.lower(StrategyOpenTrade.status) == "open")
    if sym_list:
    ...
    if open_ids:
        q_sot = select(StrategyOpenTrade).where(StrategyOpenTrade.public_id.in_(open_ids))
        if sym_list:
    ...
    for key, t in latest_by_open.items():
        ...
        sot = sot_map.get(key)
        if sot:
            side_o = (str(getattr(sot, "side", "")).lower() if hasattr(sot, "side") else "")
            ...
            markers.append(Marker(
                ...
                kind="open",
                side=("long" if is_long_o else "short") if side_o in ("long", "short") else None,
        ...
        markers.append(Marker(
            ...
            id=str(t.public_id),
            kind="close",
            side=("long" if is_long_c else "short") if side_c in ("long", "short") else None,
...
async def overview_public(
    ...
    open_count = await db.scalar(
        select(func.count()).select_from(StrategyOpenTrade)
        .where(func.lower(StrategyOpenTrade.status) == "open")
    ) or 0
    ...
    total_30 = await db.scalar(
        select(func.count()).select_from(StrategyTrade)
        .where(StrategyTrade.timestamp >= since_30d)
    ) or 0
    wins_30 = await db.scalar(
        select(func.count()).select_from(StrategyTrade)
        .where(StrategyTrade.timestamp >= since_30d)
    ...
    return {
        ...
        "max_dd_30d": None,
        "sharpe_30d": None,
        "last_signal_at": last_sig.isoformat().replace("+00:00", "Z") if last_sig else None,
    }
